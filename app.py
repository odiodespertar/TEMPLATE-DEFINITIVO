...
def gen_poligonos(data_target=None):
    ...
    fila_inner = f'''
    <tr class="calc-row">
        <td class="u-manual-cell" style="background: #fcfbc7; border: 0.6px solid #000000; padding: 2px; width: 105px; min-width: 105px; max-width: 105px;">
            <div style="{div_flex}">
        ...
        <td class="spr-real-cell" style="background: #FFFFFF; border: 0.6px solid #000000; padding: 2px; width: 135px; min-width: 135px; max-width: 135px;">
            <div style="{div_flex}">
    ...
    for i in range(1, 11):
        ...
        <div class="poligono-bloque" style="margin-bottom:12px; box-shadow: none; border-radius: 0px; overflow: hidden; background: #ededed; border: 1.5px solid #000000;">           
            <table style="width: 100%; border-collapse: collapse; border: 1.5px solid #000000;">
                ...
                <tbody>
                    <tr class="calc-row"> 
                        <td rowspan="5" contenteditable="true" style="background: #ededed; font-weight:bold; text-align:center; border: 1px solid #000000; padding: 5px; color:#333;">{nombre_final}</td>
...
<div style="margin-top:15px; display:flex; flex-direction:column; gap:10px;">
    ...
    <details style="border:1px solid #696969; border-radius:10px; background:#f8f8f8; overflow:hidden;">
        ...
        <summary style="
            ...
            font-size:14px;
            padding:12px;
            background:linear-gradient(180deg, #6065f7 0%, #6495ED 100%);
            color: #ffffff;
            user-select:none;
        ">
    ...
    <details style="border:1px solid #696969; border-radius:10px; background:#f8f8f8; overflow:hidden;">
        ...
        <summary style="
            ...
            font-size:14px;
            padding:12px;
            background:linear-gradient(180deg, #6A5ACD 0%, #9370D8 100%);
            color: #ffffff;
            user-select:none;
        ">
...
if (delta > 0 && left <= 0 && esCAR) {{
  ...
  function recalc() {{
        ...
        // 🔄 TRADUCTOR INTELIGENTE DE IDS OPERATIVOS (Evita que se rompan los botones)
        let tabIdSuperior = currentTab; // Para la tabla SCHED de arriba
        let tabIdInferior = currentTab; // Para las tablas de polígonos de abajo

        // Normalizamos los IDs si JavaScript recibe el número en lugar del texto
        if (currentTab === '2' || currentTab === 2) {{
            ...
            tabIdSuperior = '5';
            tabIdInferior = 'PREC'; // O el texto exacto que use tu div en el HTML
        }} else if (currentTab === '4' || currentTab === 4) {{
            tabIdSuperior = '4';
            tabIdInferior = 'SDE';
        }}
        ...
        // 1. Capturar datos de la flota (Tabla superior SCHED)
        ...
        // 2. Primer barrido por los polígonos (Tabla inferior usando tabIdInferior)
        let contenedorPolis = document.getElementById('polys-' + tabIdInferior);
        if (contenedorPolis) {{
        ...
        if (contenedorPolis) {{
            contenedorPolis.querySelectorAll('.poligono-bloque').forEach(bl => {{
                ...
                // 🚨 COMPROBACIÓN DE SPR REAL MÍNIMO Y COLOR DE ALERTA 🚨
                bl.querySelectorAll('tbody tr.calc-row').forEach(r => {{
                    let u = parseInt(r.querySelector('.u-manual').innerText) || 0;
                    ...
                    if(s !== "SELECCIONAR..." && fleet[s]) {{
                        ...
                        // 🔥 Si el volumen asignado por unidad cae por debajo de su mínimo, se enciende la alerta roja
        ...
        // 5. Actualizar componentes alternos
...
// ==========================================
    ...
    function toggleTools() {{
        ...
        if (!herramientasVisibles) {{
            ...
            boton.style.background = 'linear-gradient(180deg, #A9A9A9 0%, #808080 100%)'; 
